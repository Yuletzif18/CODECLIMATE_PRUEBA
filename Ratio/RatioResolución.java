...
public class PruebasCodigo {
    ...
    public static void main(String[] args) {
        ...
        calcularNPS();
        calcularRatioResolucion();
        analizarTendenciasErrores();
    }
    ...
    private static void calcularRatioResolucion() {
        int erroresReportados = contarErrores("errores_reportados.txt");
        int erroresResueltos = contarErrores("errores_resueltos.txt");

        if (erroresReportados > 0) {
            double ratioResolucion = (double) erroresResueltos / erroresReportados;
            JOptionPane.showMessageDialog(null, "Ratio de Resolución de Errores: " + ratioResolucion);
        } else {
            JOptionPane.showMessageDialog(null, "No se encontraron errores reportados para calcular la ratio.");
        }
    }

    private static int contarErrores(String archivo) {
        int contador = 0;
    ...
    private static void analizarTendenciasErrores() {
        int erroresReportados = contarErrores("errores_reportados.txt");
        int erroresResueltos = contarErrores("errores_resueltos.txt");

        // Analiza la tendencia a lo largo del tiempo
        // Aquí solo mostramos los datos, pero puedes hacer un análisis más detallado

        JOptionPane.showMessageDialog(null, "Número de Errores Reportados: " + erroresReportados + "\nNúmero de Errores Resueltos: " + erroresResueltos);
    }
}
